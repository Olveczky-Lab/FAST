param($hostname,$minclussize,$sfname,$stfname,$clusfname,$outfname,$numblocks,$nchans)
	...
	for ($j=0;$j -lt $workernodes.length;$j++) {
		...
		for ($i=0;$i -lt 2;$i++) {
			iex "& '$prc' http://$workernode username password $cmd 'TerminalWorker $minclussize $hostname $sfname $stfname $clusfname $outfname $nchans $workerendpoint $j-$i' C:\Titanic\Clusterer"
		}
	}
