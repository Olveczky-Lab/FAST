...
let getTTLChangesInRange hostname fname t0 dt =
    let inline evenize x = if (x%2UL=0UL) then x else x-1UL
    let first = getOffset hostname fname t0 |> evenize
    let last = getOffset hostname fname (t0+dt)
...
let getStart (videofnum:int64) (ephysfnum:int64) syncch videopath ephyspath = 
    ...
    let first,ttlchanges = getTTLChangesInRange Local ephysfname ((offset-30.0)|>t2s) (60.0|>t2s)
...
let getAlignedFrameTimes (videofnum:int64) (ephysfnum:int64) syncch videopath ephyspath initoffset frameinterval = 
    ...
    let framenums =
        ...
        Array.init ((Array.length tmp)/2) (fun i -> tmp.[2*i] - tmp.[0] + initoffset |> int)

    let offset = (videofnum-ephysfnum|>float)/10000000.
    let inline t2s t = max (t*30000.) 0. |>uint64
    let first,ttlchanges = getTTLChangesInRange Local ephysfname ((offset-30.0)|>t2s) (60.0|>t2s)
    ...
    | Some o ->
        let allnegedges = 
            let n = getSize Local ephysfname |> Async.RunSynchronously |> fun x -> x/8L|>uint64
            let first = first+((o+1)*2+1|>uint64)
            let cnt = min ((framenums.[Array.length framenums-1]+1000)*2) (n-first|>int)
            getTTLChanges Local ephysfname first cnt
            |> selecteven
            |> Array.toList 
        let rec removenoise xs ys = 
            ...
            | _ -> ys
        let goodnegedges,framenums =
            removenoise allnegedges [] |> List.rev |> List.toArray
            |> fun x -> 
                let n = min (Array.length x) framenums.[Array.length framenums-1]
                x.[0..n-1],framenums.[0..n-1]
...
let getVideoFiles frames (t0:uint64) (dt:uint64) =
    let inline closest t = frames |> Array.map fst |> Array.mapi (fun i (x:uint64) -> (i,(x|>int64)-(t|>int64)|>abs)) |> Array.minBy snd |> fst
...
let ffmpegargs height datafile datawidth videofiles nframes outfile frate = 
    ...
    let sfilter = 
        sprintf "color=white@0.75:%dx%d[c];[0:0][c]overlay=format=yuv444:x=%.2f*t,pad=%d:%d:0:0:white[y0];%s;%s;%s"
            datawidth height ((datawidth|>float)*frate/(nframes|>float)) (datawidth+cumw.[Array.length cumw-1]) height
            sselect sconcat soverlay 
    sprintf "-loop 1 -r %.2f -i %s %s -filter_complex \"%s\" -frames %d -y %s"
        frate datafile sinputs sfilter nframes outfile

let smoothTTL window (xs:uint64[]) =
    let n = Array.length xs
    let rec loop ys i =
        if (i < n-1) then 
            if xs.[i+1]-xs.[i] < window then loop ys (i+2) else loop (xs.[i]::ys) (i+1)
        else ys
    xs.[n-1]::loop [] 0 |>List.rev |> List.toArray

let saveAvgTapFrame ephyspath ephysfnum videopath videofnum syncch offset tapch tmppath (outdown,outup) =
    if Directory.Exists(tmppath)=false then Directory.CreateDirectory(tmppath)|>ignore
    let frames = 
        getAlignedFrameTimes videofnum ephysfnum syncch videopath ephyspath offset (1./120.) |> Option.get
    ...
    let x = getTTLChangesInRange Local (sprintf @"%s\%d\TTLChanges\Ch_%d" ephyspath ephysfnum tapch) t0 (t1-t0) |> snd |> smoothTTL 60UL
    let f outfile a = 
        a|>selecteven|>Array.choose getFrame
        |>fun x -> x.[0..(min 100 (Array.length x))-1]
        |>Array.iteri (fun i (f,s) ->
            let args = sprintf @"-r 1 -i %s\%d\%d.mkv -ss %d -frames 1 -y %s\tmp_%d_%05d.png" videopath videofnum f s tmppath offset i
            let p = new Process(StartInfo=
                ProcessStartInfo(@"\\140.247.178.88\Rajesh\ffmpeg\bin\ffmpeg.exe",args,
        ...
        let p = 
            new Process(StartInfo=
                ProcessStartInfo(@"\\140.247.178.88\Rajesh\ImageMagick\convert.exe",
                                 sprintf @"tmp_%d_*.png -evaluate-sequence mean %s" offset outfile,
                                 WorkingDirectory=tmppath,
                                 UseShellExecute=false))
        p.Start()|>ignore
        p.WaitForExit()
        Directory.GetFiles(tmppath,sprintf "tmp_%d_*.png" offset)|>Array.iter(fun x -> File.Delete(x))
    f outdown x
    f outup x.[1..]
