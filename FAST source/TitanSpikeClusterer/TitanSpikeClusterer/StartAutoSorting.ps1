param($cmdname,$hostname,$fpath,$nchans,$l2clustemp,$maxtempnum,$numblocks)
	...
	for ($j=0;$j -lt $workernodes.length;$j++) {
		...
		for ($i=0;$i -lt 4;$i++) {
			iex "& '$prc' http://$workernode Administrator olabmstr!1 $cmd '$($cmdname)Worker $hostname $fpath $nchans $l2clustemp $maxtempnum $workerendpoint $j-$i' C:\Titanic\Clusterer"
		}
	}
