...
if (!$outpath) {
	...
	if (!$hostname) {
		...
		for ($j=1;$j -le 5;$j++) {
			for ($i=0;$i -lt 4;$i++) {
				iex "& '$prc' http://192.168.0.20$j`:8000 Administrator olabmstr!1 $cmd 'RefWorker $hostname $settingsfile $workerendpoint $datafile $outpath $j-$i' C:\Titanic\Snippeter"
			}
		}
		$serverjob = Start-Job -ScriptBlock {
			param($cmd,$hostname,$settingsfile,$serverendpoint)
			$sw = [Diagnostics.Stopwatch]::StartNew()
			iex "& '$cmd' RefServer $hostname $settingsfile $serverendpoint"
			$sw.Stop()
